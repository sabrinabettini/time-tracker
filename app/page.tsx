...
export default function Home() {
  ...
  async function deleteEntry(entry: Entry) {
    const confirmed = window.confirm(
      `Delete “${entry.task}” for ${entry.client}? This cannot be undone.`,
    );
    if (!confirmed) return;
    setSaving(true);
    setError('');
  ...
  function exportEvidencePdf(records: Entry[] = reportEntries, startDate: Date = reportWeekStart) {
    ...
    const ratesHeading = document.createElement('div'); ratesHeading.className = 'rate-title'; ratesHeading.textContent = 'Summary by rate'; summary.append(ratesHeading);
    Array.from(new Map(records.map((entry) => [`${entry.rateName}|${entry.hourlyRateCents}`, entry])).values()).forEach((rateEntry) => {
      ...
      const name = document.createElement('span'); name.className = 'rate-name'; const rateLabel = document.createElement('b'); rateLabel.textContent = rateEntry.rateName; const fee = document.createElement('small'); fee.textContent = `${moneyFormatter.format(rateEntry.hourlyRateCents / 100)}/hr`; name.append(rateLabel, fee);
      const activities = document.createElement('span'); activities.textContent = `${matching.length} ${matching.length === 1 ? 'activity' : 'activities'}`;
      const time = document.createElement('strong'); time.textContent = formatDuration(minutes);
      const charged = document.createElement('strong'); charged.textContent = moneyFormatter.format((minutes / 60) * (rateEntry.hourlyRateCents / 100));
      row.append(name, activities, time, charged); summary.append(row);
    });
    body.append(summary);
    records.forEach((entry) => {
      ...
      if (entry.commitUrl || entry.screenshotUrl) {
        ...
        const heading = document.createElement('div'); heading.className = 'evidence-title'; heading.textContent = 'Evidence & references'; evidence.append(heading);
        const addLink = (label: string, url: string) => { const labelElement = document.createElement('div'); labelElement.className = 'evidence-label'; labelElement.textContent = label; const link = document.createElement('a'); link.href = url; link.target = '_blank'; link.rel = 'noreferrer'; link.textContent = url; evidence.append(labelElement, link); };
        if (entry.commitUrl) addLink('GitHub evidence', entry.commitUrl);
        if (entry.screenshotUrl) { addLink('Screenshot evidence', entry.screenshotUrl); const image = document.createElement('img'); image.src = entry.screenshotUrl; image.alt = `Screenshot evidence for ${entry.task}`; image.onerror = () => { image.remove(); const unavailable = document.createElement('div'); unavailable.className = 'muted'; unavailable.textContent = 'Screenshot preview unavailable; use the link above.'; evidence.append(unavailable); }; evidence.append(image); }
